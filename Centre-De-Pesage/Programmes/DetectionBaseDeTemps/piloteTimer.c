...
// Fonction nécessaire au fonctionnement de la base de temps


size_t piloteTimer_timer1;

void (*piloteTimer_execute)(void);

void piloteTimer_neFaitRien(void);
void piloteTimer_gere(size_t timer_id, void * user_data);
...
{
    stop_timer(piloteTimer_timer1);
    finalize();
}

void piloteTimer_initialise(void)
{
    piloteTimer_execute = piloteTimer_neFaitRien;
    initialize();
...
void piloteTimer_DemarreAvantWhile(void)
{
    ...
    piloteTimer_timer1 = start_timer(PILOTETIMER_PERIODE_EN_MS, piloteTimer_gere, TIMER_PERIODIC, NULL);
}
