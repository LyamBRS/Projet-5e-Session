...
void ServiceCommunication_RXParsingHandler(void)
{
    //########################################## LOCAL VAR
    //##########################################
    //unsigned char longueur = 0x08; // On force la longueur en unsigned char
    // CAN DATA IS AVAILABLE
    if(CHECK_MODULE_CAN_RECEPTION) // Doit etre a TRUE
    {
        unsigned char bruh;
        unsigned long adresseRECEP = 0x00;
        piloteCAN_recoitUnMessage(&adresseRECEP ,piloteCAN.readBuffer, &bruh);
        Parse_CanBusReceptions(MODULE_CAN_RX_BUFFER);
        for(int i = 0; i < 8; i++)
        {
            piloteCAN.readBuffer[i] = 0x00;
        }
