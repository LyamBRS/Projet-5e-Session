 ...
 */
#define SCOMMS_PARSE_RX_BUFFER_ADR SERVICECOMMUNICATION_GERE1
/**
 * @brief The address from the timebase buffer (index) which will be used to put
 * \ref ServiceCommunication_TXParsingHandler() in
 */
#define SCOMMS_PARSE_TX_BUFFER_ADR SERVICECOMMUNICATION_GERE2
/**
 ...
 */
#define CHECK_MODULE_CAN_RECEPTION interfaceCAN1_messageDisponible()
/**
 * @brief Statement which is true only if a CAN message addressed to your
 * module is available.
 * It is false if no messages have been received.
...
typedef struct
{
    ...
    unsigned char Mode;
    /**
     * @brief Value from 0 to 254 either in KG or in Lbs. Set to \ref UNUSED_
     * by default. If not set to \ref UNUSED_, it will be sent each available
     * CAN slots.
     */
