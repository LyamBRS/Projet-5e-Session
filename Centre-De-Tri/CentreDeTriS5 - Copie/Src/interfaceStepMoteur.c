...
void interfaceStepMoteur_gere(void)
{
  ...
  if (compteTemps >= INTERFACEASCENSEUR_VITESSE)
  {
    ...
    compteTemps = 0;
    if (tabStepMoteur[0][overallStep - (overallStep/8)])piloteStepMoteur_EcritSortie(PILOTESTEPMOTEUR_STEP0, PILOTESTEPMOTEUR_ACTIVE);
    else piloteStepMoteur_EcritSortie(PILOTESTEPMOTEUR_STEP0, PILOTESTEPMOTEUR_DESACTIVE);

    if (tabStepMoteur[1][overallStep - (overallStep/8)])piloteStepMoteur_EcritSortie(PILOTESTEPMOTEUR_STEP1, PILOTESTEPMOTEUR_ACTIVE);
    else piloteStepMoteur_EcritSortie(PILOTESTEPMOTEUR_STEP1, PILOTESTEPMOTEUR_DESACTIVE);

    if (tabStepMoteur[2][overallStep - (overallStep/8)])piloteStepMoteur_EcritSortie(PILOTESTEPMOTEUR_STEP2, PILOTESTEPMOTEUR_ACTIVE);
    else piloteStepMoteur_EcritSortie(PILOTESTEPMOTEUR_STEP2, PILOTESTEPMOTEUR_DESACTIVE);

    if (tabStepMoteur[3][overallStep - (overallStep/8)])piloteStepMoteur_EcritSortie(PILOTESTEPMOTEUR_STEP3, PILOTESTEPMOTEUR_ACTIVE);
...
bool interfaceStepMoteur_requete(unsigned char requete)
{
  ...
  {
    positionVoulue = INTERFACESTEPMOTEUR_POSITION_BASSE;
  }
